...
public void tampil_masuk(){
        model = (DefaultTableModel) tabelparkir.getModel();
        try {
            konek.st = konek.conn.createStatement();
            String query = "SELECT * FROM parkir WHERE biaya_parkir=0 ORDER BY id DESC LIMIT 25";
            konek.rs = konek.st.executeQuery(query);
            while (konek.rs.next()) {
                String id = konek.rs.getString("id");
                String nopol = konek.rs.getString("nopol");
                String tipe = konek.rs.getString("tipe");
                String tanggal = konek.rs.getString("tanggal");
                String jam_masuk = konek.rs.getString("jam_masuk");
                
                String [] data = {id, nopol, tipe, tanggal, jam_masuk};
                model.addRow(data);
            }
        } catch (Exception e) {
...
public void tampil_keluar(){
        ...
        try {
            konek.st = konek.conn.createStatement();
            String query = "SELECT * FROM parkir WHERE biaya_parkir <> 0 ORDER BY tanggal DESC,jam_keluar DESC LIMIT 15";
            konek.rs = konek.st.executeQuery(query);
            while (konek.rs.next()) {
                String jam_keluar = konek.rs.getString("jam_keluar");
                String biaya_parkir = konek.rs.getString("biaya_parkir");
                Double biaya2 = Double.parseDouble(biaya_parkir);
                String biaya_final = (rupiah.format(biaya2));
                
                String [] data = { jam_keluar, biaya_final};
                model.addRow(data);
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Gagal ambil data"+e);
